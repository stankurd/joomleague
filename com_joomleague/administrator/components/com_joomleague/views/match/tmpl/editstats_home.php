		...
		foreach ( $this->homeRoster as $row ) {
			...
			<tr id="row<?php echo $i;?>" class="statrow">
				<td style="text-align: left;"><input type="hidden"
					name="teamplayer_id[]" value="<?php echo $row->tpid;?>" /> <input
					type="hidden" name="projectteam_id[]"
					value="<?php echo $row->projectteam_id;?>" /> <input
					type="checkbox" class="statcheck" id="cb<?php echo $i;?>"
					name="cid[]" value="<?php echo $i;?>" /></td>
				<td style="text-align: left; width: 200px;">
				<?php echo JoomleagueHelper::formatName(null, $row->firstname, $row->nickname, $row->lastname, $this->default_name_format); ?>
				</td>
				...
				<?php if (!$ev->getCalculated() && $ev->position_id == $position->value): ?>
					<td style="text-align: center;">
						<?php $val = isset($this->playerstats[$row->projectteam_id][$row->tpid][$ev->id]) ? $this->playerstats[$row->projectteam_id][$row->tpid][$ev->id] : ""; ?>
							<input type="text" size="3" class="inputbox stat"
					title="<?php echo $ev->name; ?>"
	...
	<table>
		...
		<?php
			...
			<tr id="staffrow<?php echo $j;?>" class="staffstatrow">
				<td style="text-align: left;"><input type="hidden"
					name="team_staff_id[]" value="<?php echo $row->team_staff_id;?>" />
					<input type="hidden" name="sprojectteam_id[]"
					value="<?php echo $row->projectteam_id;?>" /> <input
					type="checkbox" class="staffstatcheck" id="staffcb<?php echo $i;?>"
				...
				<?php if (!$ev->getCalculated() && $ev->position_id == $position->value): ?>
					<td>
						...
						<input type="text" size="3" class="inputbox staffstat"
					title="<?php echo $ev->name; ?>"
					name="staffstat<?php echo $row->team_staff_id;?>_<?php echo $ev->id; ?>"
					value="<?php echo $val; ?>" />
				</td>
					<?php endif;	?>
				<?php endforeach; ?>
