...
function observeClubIcons(moduleid, imgclass, itemcnt, wdiff)
  {
    ...
    document.getElementById(holderid).setStyle('visibility', 'hidden');
    new Asset.images(jQuery('#'+holderid+' '+imgclass), {
      onComplete: function(){
          document.getElementById(holderid).setStyle('visibility', 'visible');
      }
    });
    var maxheight = 0;
    var holderpartwidth = (document.getElementById(holderid).getSize().size.x / itemcnt).toInt();
    jQuery('#'+holderid+' '+imgclass).each(function(el, index){ 
    ...
    jQuery('#'+holderid+' tr').each(function(el){ el.setStyle('height', maxheight+"px"); });
  }