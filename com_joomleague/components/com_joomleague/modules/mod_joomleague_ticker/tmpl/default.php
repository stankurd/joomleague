...
foreach ($matches as $match)
{
	...
	switch ($urlformat)
	{
		...
		case 2:
			...
			break;
		case 3:
			$plan_link = JoomleagueHelperRoute::getTeamPlanRoute( $match->project_id, $match->team1_id, $match->division_id);
	...
	switch ($outline)
	{
		...
		case 0:
			...
			$textdiv .= '<br />';
			$textdiv .=		'<div>';
			$textdiv .= 		'<div >' . $home_name. '</div>';
			$textdiv .= 		'<div>' . $urlres. $match->team1_result. $urlresb . '</div>';
			$textdiv .= 		'<div><center>' . $teamseparator . '</center></div><div style="width:20%;"></div>';
			$textdiv .= 		'<div>' . $away_name . '</div>';
			$textdiv .= 		'<div>' . $urlres. $match->team2_result. $urlresb . '</div>';
			$textdiv .= 	'</div>';
			$textdiv .=	'</div>';
			break;
