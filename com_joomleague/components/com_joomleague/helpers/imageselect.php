...
class ImageSelect
{
	...
	function getSelector( $fieldname, $fieldpreview_name, $type, $value, $default = '',$control_name ='')
	{
		...
		$baseFolder = Uri::root();//.'images/com_joomleague/'.ImageSelect::getfolder($type);
		...
		{
			$('a_' + field).value = 'images/com_joomleague/" . ImageSelect::getfolder( $type ) . "/'+image;
			$('a_' + field + '_name').value ='images/com_joomleague/" . ImageSelect::getfolder( $type ) . "/'+imagename;
			$('a_' + field + '_name').fireEvent('change');
      if($('params' + field)) {
        $('params' + field).value = 'images/com_joomleague/" . ImageSelect::getfolder( $type ) . "/'+imagename;
      }
			$('a_' + field + '_name').fireEvent('change');
			$('sbox-window').close();
		}
		...
		Text::_( 'JL_GLOBAL_UPLOAD' ) . "\" href=\"$link\" rel=\"{handler: 'iframe', size: {x: 800, y: 500}}\">" .
		Text::_( 'JL_GLOBAL_UPLOAD' ) . "</a></div></div>\n";
		$imageselect .=	"<div class=\"button2-left\"><div class=\"blank\"><a class=\"modal\" title=\"" .
		Text::_( 'JL_GLOBAL_SELECT_IMG' ) . "\" href=\"$link2\" rel=\"{handler: 'iframe', size: {x: 800, y: 500}}\">" .
		Text::_( 'JL_GLOBAL_SELECT_IMG' )."</a></div></div>\n";
		$imageselect .=	"<div class=\"button2-left\"><div class=\"blank\"><a title=\"" .
		Text::_( 'JL_GLOBAL_SELECT_IMG' ) . "\" href=\"#\" onclick=\"reset_" . $fieldname . "();\">" . Text::_( 'JL_GLOBAL_RESET' ) . "</a></div></div>\n";
		$imageselect .=	"<div class=\"button2-left\"><div class=\"blank\"><a title=\"" .
		Text::_( 'JL_GLOBAL_CLEAR' ) . "\" href=\"#\" onclick=\"clear_" . $fieldname . "();\">" . Text::_( 'JL_GLOBAL_CLEAR' ) . "</a></div></div>\n";
	...
	function check( $file )
	{
		...
		{
			Factory::getApplication()->enqueueMessage( 100, Text::_( 'JL_IMAGEHANDLER_UPLOAD_FAILED' ) . ': ' . htmlspecialchars($file['name'], ENT_COMPAT, 'UTF-8' ),'watning' );
			return false;
		}
		...
		{
		    Factory::getApplication()->enqueueMessage( 100, Text::_( 'JL_IMAGEHANDLER_ERROR1' ) . ': ' . htmlspecialchars( $file['name'], ENT_COMPAT, 'UTF-8' ),'warning' );
			return false;
		}
		...
		{
		    Factory::getApplication()->enqueueMessage( 100, Text::_( 'JL_IMAGEHANDLER_ERROR2' ) . ': ' . htmlspecialchars( $file['name'], ENT_COMPAT, 'UTF-8' ),'warning' );
			return false;
		}
		...
		foreach( $html_tags as $tag )
		{
			...
			{
			    Factory::getApplication()->enqueueMessage( 100, Text::_( 'JL_IMAGEHANDLER_IE_WARN' ),'warning' );
				return false;
			}
		}
