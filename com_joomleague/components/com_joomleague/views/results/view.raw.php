...
class JoomleagueViewResults extends JLGView
{
	...
	public function display($tpl = null)
	{
		...
		// Set page title
		$pageTitle = Text::_('COM_JOOMLEAGUE_RESULTS_PAGE_TITLE');
	...
	public function getTeamClubIcon($team,$type=1,$attribs=array())
	{
		...
		if ($type==1)
		{
			...
			{
				$image=HTMLHelper::image('images/com_joomleague/database/placeholders/placeholder_small.gif',$title,$attribs);
			}
		}
		elseif ($type==2 && !empty($team->country))
		{
			...
			if (empty($image))
			{
				$image=HTMLHelper::image('images/com_joomleague/database/placeholders/placeholder_flags.png',$title,$attribs);
			}
		}
		else
		{
	...
	public function editPartResults($i,$match,$backend=false)
	{
		...
		$imgTitle=Text::_('COM_JOOMLEAGUE_ADMIN_EDIT_MATRIX_ROUNDS_PART_RESULT');
		$desc=HTMLHelper::image("media/com_joomleague/jl_images/sort01.gif", $imgTitle, array("border" => 0,"title" => $imgTitle));
		...
		if ($this->project->allow_add_time)
		{
			if($match->match_result_type >0){
				echo Text::_('COM_JOOMLEAGUE_RESULTS_OVERTIME').':';
				echo '<input type="text" style="font-size:9px;"';
			...
			if($match->match_result_type == 2){
				echo '<br />';
				echo Text::_('COM_JOOMLEAGUE_RESULTS_SHOOTOUT').':';
				echo '<input type="text" style="font-size:9px;"';
	...
	*/
	...
	{
		$imgTitle=Text::_('COM_JOOMLEAGUE_RESULTS_SHOW_EVENTS_OF_MATCH');
		$attribs=array(	"title" => $imgTitle,
		 		"id" => 'events-'. $match_id,
		 		"class" => "eventstoggle");
		$img=HTMLHelper::image('media/com_joomleague/jl_images/events.png',$imgTitle,$attribs);
		return $img;
	}
	...
	function showMatchRefereesAsTooltip(&$game)
	{
		if ($this->config['show_referee'])
		{
			...
			<span class="hasTip"
				...
				src="<?php echo Uri::root(); ?>media/com_joomleague/jl_images/icon-16-Referees.png"
				alt="" title="" /> </span>
	...
	function showReportDecisionIcons(&$game)
	{
		...
		if ((($game->show_report) && (trim($game->summary) != '')) || ($game->alt_decision) || ($game->match_result_type > 0))
		{
			...
			$report_link,
			HTMLHelper::image($img,$imgTitle,array("border" => 0,"title" => $imgTitle)),
	...
	function showEventsContainerInResults($matchInfo,$projectevents,$matchevents,$substitutions=null,$config)
	{
		...
		if ($this->config['use_tabs_events'])
		{
			...
			$output  .= HTMLHelper::_('bootstrap.startTabSet', $selector, array('active'=>!empty($substitutions) ? 'substitutions' : 'tab1'));
			$i        = 1;
			foreach ($projectevents AS $event)
			{
				...
				$output .= HTMLHelper::_('bootstrap.addTab', $selector, 'tab'.$i++, $txt_tab);
			...
			if (!empty($substitutions))
			{
				...
				$imgTitle	= Text::_('COM_JOOMLEAGUE_IN_OUT'); $imgTitle2=array(' title' => $imgTitle);
				$txt_tab	= HTMLHelper::image($pic_tab,$imgTitle,$imgTitle2);

				$imgTime=HTMLHelper::image($pic_time,Text::_('COM_JOOMLEAGUE_MATCHREPORT_SUBSTITUTION_MINUTE'),array(' title' => Text::_('COM_JOOMLEAGUE_MATCHREPORT_SUBSTITUTION_MINUTE')));
				$imgOut=HTMLHelper::image($pic_out,Text::_('COM_JOOMLEAGUE_MATCHREPORT_SUBSTITUTION_WENT_OUT'),array(' title' => Text::_('COM_JOOMLEAGUE_MATCHREPORT_SUBSTITUTION_WENT_OUT')));
				$imgIn=HTMLHelper::image($pic_in,Text::_('COM_JOOMLEAGUE_MATCHREPORT_SUBSTITUTION_CAME_IN'),array(' title' => Text::_('COM_JOOMLEAGUE_MATCHREPORT_SUBSTITUTION_CAME_IN')));

				$output .= HTMLHelper::_('bootstrap.addTab', $selector, 'substitutions', $txt_tab);
				$output .= '<table class="matchreport" border="0">';
				$output .= '<tr>';
				$output .= '<td class="list">';
				$output .= '<ul>';
		...
		else
		{
			...
			foreach ((array) $matchevents AS $me)
			{
				if ($me->ptid == $matchInfo->projectteam1_id)
				{
				    ...
				    { 
				    	$event_icon = Uri::root().$projectevents[$me->event_type_id]->icon;
