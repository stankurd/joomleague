...

{
    ...
    function __construct()
    {
        ...
        $this->alltimepoints = $app->input->getVar("points", 0);
       /* 
        $menu = $app->getMenu();
        $item = $menu->getActive();
        $params = $menu->getParams($item->id);
*/
        $show_debug_info = ComponentHelper::getParams('com_joomleague')->get('show_debug_info',
            0);
        if ($show_debug_info) {
...
foreach ($newparams['data'] as $key => $value ) {
        ...
        {
            $strXmlFile = JLG_PATH_SITE.'/extensions/rankingalltime/settings/default/rankingalltime.xml';
            //$strXmlFile = JLG_PATH_SITE .'/settings/default/rankingalltime.xml';    
            // get the JForm object
            $form = Form::getInstance('jlattform', $strXmlFile);
...
//         echo ' -> '. $field->input.'<br>';
    ...
    function getAllTeamsIndexedByPtid($project_ids)
    {
        ...
        $app->enqueueMessage(Text::_('Uwzgledniono lacznie '.$count_teams.' Zespol/Sezon !'),'Notice');
    ...
    function getAllTeams($project_ids)
    {
        ...
        $query
               ...
               // ->select('u.username')
                ...
                ->select('CASE WHEN CHAR_LENGTH( p.alias ) THEN CONCAT_WS( \':\', p.id, p.alias ) ELSE p.id END AS project_slug')
                ->select('CASE WHEN CHAR_LENGTH( t.alias ) THEN CONCAT_WS( \':\', t.id, t.alias ) ELSE t.id END AS team_slug')
                ->select('CASE WHEN CHAR_LENGTH( d.alias ) THEN CONCAT_WS( \':\', d.id, d.alias ) ELSE d.id END AS division_slug')
                ->select('CASE WHEN CHAR_LENGTH( c.alias ) THEN CONCAT_WS( \':\', c.id, c.alias ) ELSE c.id END AS club_slug')
                ->from('#__joomleague_project_team tl')
                ->leftJoin('#__joomleague_team t ON tl.team_id = t.id')
                //->leftJoin('#__users u ON tl.admin = u.id')
                ...
                ->where('tl.project_id IN (' . $project_ids . ')');
                try {
        $db->setQuery($query);
    ...
    $query
            ->select('m.id')
            ->select('m.projectteam1_id')
            ->select('m.projectteam2_id')
            ->select('m.team1_result AS home_score')
            ->select('m.team2_result AS away_score')
            ->select('m.team1_bonus AS home_bonus')
            ->select('m.team2_bonus AS away_bonus')
            ->select('m.team1_legs AS l1')
            ->select('m.team2_legs AS l2')
            ->select('m.match_result_type AS match_result_type')
            ->select('m.alt_decision as decision')
            ->select('m.team1_result_decision AS home_score_decision')
            ->select('m.team2_result_decision AS away_score_decision')
            ->select('m.team1_result_ot AS home_score_ot')
            ->select('m.team2_result_ot AS away_score_ot')
            ->select('m.team1_result_so AS home_score_so')
            ->select('m.team2_result_so AS away_score_so')
            ->select('t1.id AS team1_id')
            ->select('t2.id AS team2_id')
            ->select('r.id as roundid')
            ->select('m.team_won')
            ->select('r.roundcode')
            ->from('#__joomleague_match m')
            ->innerJoin('#__joomleague_project_team AS pt1 ON m.projectteam1_id = pt1.id')
            ->innerJoin('#__joomleague_team t1 ON pt1.team_id = t1.id')
            ->innerJoin('#__joomleague_project_team AS pt2 ON m.projectteam2_id = pt2.id')
            ->innerJoin('#__joomleague_team t2 ON pt2.team_id = t2.id')
            ->innerJoin('#__joomleague_round AS r ON m.round_id = r.id')
            ->where('((m.team1_result IS NOT NULL AND m.team2_result IS NOT NULL) OR (m.alt_decision=1))')
            ->where('m.published = 1')
            ->where('pt1.project_id IN ('.$projects.')')
            ->where('(m.cancel IS NULL OR m.cancel = 0)')
            ->where('m.projectteam1_id>0 AND m.projectteam2_id>0 ');
            try {
    ...
    $app->enqueueMessage(Text::_('Podliczono '.$count_matches.' Mecze !'),'Notice');
    ...
    function getAllProject()
    {
        ...
        $db = Factory::getDbo();
        $query = $db->getQuery(true);
        $league = $app->input->getInt("l", 0);

        if (!$league) {
            $projekt = $app->input->getInt("p", 0);
            $query->clear();
            $query
                    ->select('league_id')
        ...
        $app->enqueueMessage(Text::_('Podsunowano na podstawie '.$count_project.' Projektow/Sezonow wedlug punkacji 3,1,0 !'),'Notice');
        return $result;
    ...
    function _sortRanking(&$ranking)
    {
        ...
        $app = Factory::getApplication();
        //$order = $app->input->getVar('order', '');
        //$order_dir = $app->input->getVar('dir', 'DESC');
        $order = $app->input->request->get('order', '', 'STR');
        $order_dir = $app->input->request->get('dir', 'DESC', 'STR');
        ...
        if (!$order) {
            ...
            foreach ($arr2 as $key => $row) {
                $ranking2[$key] = ArrayHelper::toObject($row, 'JLGRankingalltimeTeam');
                $ranking[$key] = ArrayHelper::toObject($row, 'JLGRankingalltimeTeam');
            }
        ...
        if (empty($this->_criteria)) {
            ...
            foreach ($values as $v) {
                ...
                if (method_exists($this, '_cmp' . $v)) {
                    ...
                    $app->enqueueMessage(Text::_('JL_RANKING_NOT_VALID_CRITERIA') . ': ' . $v,'warning');
                }
            }
