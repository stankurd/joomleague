...
//         echo ' -> '. $field->input.'<br>';
    ...
    function getAllTeamsIndexedByPtid($project_ids)
    {
        ...
        $app->enqueueMessage(Text::_('Podliczono '.$count_teams.' Zespoł-Sezon !'),'');
    ...
    $app->enqueueMessage(Text::_('Podliczono na podstawie '.$count_matches.' Meczy wg punktacji 3/1/0 !'),'');
  ...
  where league_id = ' . $league . ' order by name ';
        ...
        $app->enqueueMessage(Text::_('Podliczono '.$count_project.' Projektow/Sezonow !'),'');
        ...
        if (empty($this->_criteria)) {
            ...
            foreach ($values as $v) {
                ...
                if (method_exists($this, '_cmp' . $v)) {
                    ...
                    //JError::raiseWarning(0, Text::_('JL_RANKING_NOT_VALID_CRITERIA') . ': ' . $v);
                }
            }
