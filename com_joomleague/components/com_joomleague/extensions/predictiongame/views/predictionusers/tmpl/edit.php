...
{
	Factory::getApplication()->redirect(str_ireplace('&layout=edit','',Uri::getInstance()->toString()),Text::_('ALERTNOTAUTH'));
}
...
<form name='adminForm' id='adminForm' method='post' >
	<table width='100%'><tr><td class='sectiontableheader'><?php echo Text::sprintf('COM_JOOMLEAGUE_PRED_USERS_EDIT_TITLE',$this->predictionGame->name); ?></td></tr></table>
	<br />
			...
			<td colspan='2'><?php
				...
				else
				{
					...
					echo	$this->predictionMember->pmRegisterDate != '0000-00-00 00:00:00' ?
							HTMLHelper::date($this->predictionMember->pmRegisterDate,Text::_('COM_JOOMLEAGUE_PRED_USERS_EDIT_LABEL_REGDATE_FORMAT')) :
				...
				foreach ($this->predictionProjectS AS $predictionProject)
				{
					...
					?><td><?php
						if ($rowspan > 1)
						{
							...
							{
								echo Text::sprintf('COM_JOOMLEAGUE_PRED_USERS_FAVTEAM_IN_PROJECT','<b>'.$predictionProjectSettings->name.'</b>');
							}
							else
							{
								echo Text::_('COM_JOOMLEAGUE_PRED_USERS_EDIT_GETPROJECT_ERROR');
							}
						}
						else {echo '&nbsp;';}
						?></td></tr><tr><?php
            ...
            echo $this->lists['champ_tipp_enabled'][$predictionProject->project_id];
					...
					?><td><?php
						if ($rowspan > 1)
						{
							if ($predictionProjectSettings = $this->model->getPredictionProject($predictionProject->project_id))
							{
								echo Text::sprintf('COM_JOOMLEAGUE_PRED_USERS_CHAMPION_IN_PROJECT','<b>'.$predictionProjectSettings->name.'</b>');
							}
							else
							{
								echo Text::_('COM_JOOMLEAGUE_PRED_USERS_EDIT_GETPROJECT_ERROR');
		...
		<tr>
			...
			<td colspan='2'>
				<img	class='imagepreview' src='<?PHP echo $this->predictionMember->picture  ; ?>' name='picture_preview'
						id='picture_preview' border='3' alt='<?php echo Text::_('COM_JOOMLEAGUE_PRED_USERS_EDIT_AVATAR_PREVIEW'); ?>' title='<?php echo Text::_('COM_JOOMLEAGUE_PRED_USERS_EDIT_AVATAR_PREVIEW'); ?>' /><br /><?php echo $this->imageselect; ?>
